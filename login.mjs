...
const gerarToken=(d,ttl=604800)=>{const raw=Buffer.from(JSON.stringify({d,exp:Math.floor(Date.now()/1000)+ttl})).toString('base64url');return raw+'.'+crypto.createHmac('sha256',SECRET).update(raw).digest('base64url');};
export default async (req)=>{
  ...
  const token=gerarToken({email,nome:u.nome,crm:u.crm||''},604800);
  return json({ok:true,nome:u.nome,token});
};
